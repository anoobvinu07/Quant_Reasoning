...
# The frequency of allele A switches from maximum (being fixed) to minimum (going extinct) when the population is very low (e.g. N=50). However, when the frequency of A is high (e.g. N=1000), the frequency is more or less stable between the ranges of 0.4 and 0.0. 


# 2. How does the initial frequency of the A allele affect the probability it will reach fixation?
# Whenever the initial frequency of allele A is high (>=0.4), the allele A tends to be fixed or remains close to the initial value. I also noticed that as soon as it is set to 0.5, more often than not they are fixed and rarely goes extinct (but it still do go extinct at times).
...
# When this mutation occurs, the allele A is almost always fixed over time, most of the time before time 40. With the mutation always results in higher frequency of allele A even when the population size was small and when the initial frequency is low.
